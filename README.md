...
<body>
    <div class="container">
        ...
        <div style="text-align: center; margin: 10px 0; padding: 10px; background: rgba(255,255,255,0.1); border-radius: 10px;">
            <div id="statusIndicator" style="font-weight: 600; color: #2c3e50;">
                🟡 Connecting...
            </div>
    ...
    <script>
        ...
        const firebaseConfig = {
            apiKey: "demo-key",
            authDomain: "study-tracker-demo.firebaseapp.com",
            databaseURL: "https://study-tracker-demo-default-rtdb.firebaseio.com/",
            projectId: "study-tracker-demo"
        };
        ...
        try {
            ...
            userId = localStorage.getItem('studyTrackerUserId');
            if (!userId) {
        ...
        window.addEventListener('online', () => {
            isOnline = true;
            document.getElementById('statusIndicator').textContent = '🟢 Online - Syncing across devices';
            syncLocalToCloud();
        });

        window.addEventListener('offline', () => {
            isOnline = false;
            document.getElementById('statusIndicator').textContent = '🔴 Offline - Local storage only';
        });
        // Study plan data
        const physicsTopics = [
        ...
        // Load saved data from cloud or localStorage
        async function loadSavedData() {
            try {
                if (database && isOnline) {
                    ...
                    if (cloudData) {
                        // Restore checkbox states from cloud
                        Object.keys(cloudData).forEach(day => {
                            const status = cloudData[day];
                        ...
                        localStorage.setItem('studyTimetableProgressJune', JSON.stringify(cloudData));
                        document.getElementById('statusIndicator').textContent = '🟢 Online - Syncing across devices';
                    } else {
        ...
        async function saveData() {
            ...
            if (database && isOnline) {
                try {
                    await database.ref(`users/${userId}/progress`).set(progressData);
                    ...
                    document.getElementById('statusIndicator').textContent = '🟡 Sync issues - Using local storage';
                }
            }
        }
        ...
        async function syncLocalToCloud() {
            ...
            const localData = localStorage.getItem('studyTimetableProgressJune');
            if (localData) {
                try {
        ...
        async function clearAllData() {
            if (confirm('Are you sure you want to clear all your progress? This will remove data from all your devices and cannot be undone!')) {
                // Clear local storage
                ...
                if (database && isOnline) {
                    try {
                        await database.ref(`users/${userId}/progress`).remove();
                        await database.ref(`users/${userId}/lastUpdated`).set(firebase.database.ServerValue.TIMESTAMP);
                    } catch (error) {
                ...
                updateStats();
                alert('All progress data has been cleared from all devices!');
            }
        }
        ...
        // Set up real-time listener for cloud updates
        if (database) {
            database.ref(`users/${userId}/progress`).on('value', (snapshot) => {
                if (snapshot.exists()) {
                    ...
                    if (JSON.stringify(cloudData) !== localData) {
                        ...
                        // Update checkboxes to match cloud data
                        document.querySelectorAll('.custom-checkbox').forEach(box => {
                            box.classList.remove('checked', 'crossed');
                        });
        ...
        setTimeout(() => {
            if (userId) {
                console.log('Your Study Tracker ID:', userId);
                console.log('Save this ID to access your data from other devices');
            }
        }, 2000);
    </script>
</body>
